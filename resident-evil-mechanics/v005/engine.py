...
class Environment:
    ...
    def extract_item(self, item, quantity):
        # Database transaction handling structural item depletion or inventory removal
...
class Player:
    def __init__(self, environment, character_data, item_properties):
        ...
        # Refactored data structure to map stackable items and dynamic balances
        self.inventory = {}
    ...
    def collect_item(self, item, quantity):
        ...
        if item_collected:
            # Upsert Logic: Creates the storage tracking entry or increments stack totals
    ...
    while True:
        ...
        while True:
            try:
                ...
                # Fetch array key match based on user selection index
                extracted_item = available_items[item_index - 1]
                requested_quantity = int(input('What quantity do you want to extract? '))
            except (ValueError, IndexError):
                print('Invalid option. Enter correct parameters.')
                input()
                update_screen()
                break
            else:
                # Validation safeguard logic checking stock threshold capabilities
                if requested_quantity <= environment.scene_items[extracted_item]:
                    leon.collect_item(extracted_item, requested_quantity)
                    print(f"{requested_quantity} {extracted_item.replace('_', ' ')} successfully extracted.")
                    input('Press Enter to continue...')
                    break
                else:
                    print('Requested quantity exceeds physical room availability.')
                    input()
